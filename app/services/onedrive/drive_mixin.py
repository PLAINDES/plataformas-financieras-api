...
from .config import GRAPH_BASE, ROOT_FOLDER, Environment, Folder

logger = logging.getLogger(__name__)


class OneDriveDriveMixin:
    """Mixin para operaciones de archivos y carpetas en OneDrive."""

    @staticmethod
    def build_path(
        env: Environment, folder: Folder, filename: Optional[str] = None
    ) -> str:
        """
    ...
    def _drive_url(
        self, path: Optional[str] = None, item_id: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code in [401, 403, 404]:
                    logger.warning(
                        f"Fallo acceso a {url}. Código {resp.status_code}. Intentando fallback a la raíz..."
                    )
                ...
                return {
                    ...
                    or data.get("parentReference", {}).get("driveId", "Unknown"),
                    "owner_email": self.config.user_email,
                    "owner_display_name": data.get("owner", {})
                    .get("user", {})
    ...
    async def ensure_folder_structure(self, structure: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            for root_name, sub_structure in structure.items():
                try:
                    # Create root folder
                    root_result = await self._ensure_folder(
                        client, headers, "root", root_name
                    )
                    ...
                    if isinstance(sub_structure, dict):
                        ...
                        for env, folders_list in sub_structure.items():
                            await self._ensure_folder(
                                client, headers, f"root:/{root_name}:", env
                            )
                        ...
                        for folder in sub_structure:
                            await self._ensure_folder(
                                client, headers, f"root:/{root_name}:", folder
                            )
        ...
        logger.info(
            f"OneDrive folder structure setup complete. Created: {len(created)}, Errors: {len(errors)}"
        )
        return {
            "created": created,
            "errors": errors,
    ...
    async def _ensure_folder(
        ...
        url = (
            f"{GRAPH_BASE}/users/{self.config.user_email}/drive/{parent_path}/children"
        )
        payload = {
            "name": folder_name,
            "folder": {},
    ...
    async def download_file(self, item_id: str) -> bytes:
        ...
        token = await self._get_token()
        url = (
            f"{GRAPH_BASE}/users/{self.config.user_email}/drive/items/{item_id}/content"
        )
    ...
    async def delete_file(self, item_id: str) -> None:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 204:
                logger.warning(
                    f"Unexpected status deleting item {item_id}: {resp.status_code}"
                )
    ...
    async def copy_file(
        ...
        target_folder_url = (
            f"{GRAPH_BASE}/users/{self.config.user_email}/drive/root:/{target_path}"
        )
