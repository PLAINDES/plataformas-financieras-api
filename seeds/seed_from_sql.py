...
def table_has_data(conn: pymysql.Connection, table_name: str) -> bool:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                f"SELECT COUNT(*) as count FROM `{table_name}` WHERE id = 1 LIMIT 1"
            )
...
def run_seed(sql_path: Path, conn: Optional[pymysql.Connection] = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            conn.commit()
            logger.info(
                f"✓ Successfully executed {sql_path.name} ({len(statements)} statements)"
            )
            return True
        finally:
            if close_conn:
    ...
    try:
        ...
        for seed_file, check_table in seeds:
            if table_has_data(conn, check_table):
                logger.info(
                    f"✓ Table '{check_table}' already has data, skipping {seed_file.name}"
                )
                skipped_count += 1
            else:
        ...
        logger.info("=" * 60)
        logger.info(
            f"Seeding complete: {executed_count} executed, {skipped_count} skipped"
        )
        logger.info("=" * 60)
        return True
