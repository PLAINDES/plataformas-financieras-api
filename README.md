...
Financiera is a comprehensive backend API built with FastAPI that powers a financial management platform and a Content Management System (CMS). It handles native financial calculations, S3-based template and media storage, PDF report generation, and an embedded chatbot service.
...
- **External Integrations**:
  - **AWS S3**: For master templates, calculation files, and extracted chart images.
...
│   │   ├── main/
│   │   │   ├── calculations/   # Native finance logic (formatters, payload, graphs, macros)
...
│   │   └── storage/        # Storage-related routes
│   ├── core/               # App configuration, constants, and security
│   ├── db/                 # Database setup and sessions
│   ├── models/             # SQLAlchemy ORM models
│   ├── repositories/       # Data Access Layer
│   └── services/           # Reusable business logic and native Valora services
...
- Advanced Financial Engine: Native Kapital and Valora calculations persisted as structured JSON and linked to S3 assets.
...
- If AWS S3 credentials fail, the endpoint returns a controlled storage error.
- If a chart fails to render or upload, the process reports the failure while preserving the available text codes.
- Calculation reports consume S3 object keys and native calculation data; no external workbook session is required.
